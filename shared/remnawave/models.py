...
Описаны только поля, которые панель действительно использует: спека 2.8.1
огромная, а генерация всех схем целиком дала бы десятки тысяч строк, которые
никто не читает. Соответствие спеке держит контрактный тест
`backend/tests/test_remnawave_contract.py` — он падает, если в новой версии
Remnawave пропал эндпоинт или поле, на которое мы опираемся.
